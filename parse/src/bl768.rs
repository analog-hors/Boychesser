...
impl BatchLoader for BatchLoader768 {
                    ...
                    for (index, &pieces) in array.iter().enumerate() {
                        for sq in pieces {
                            let (stm_index, stm_sq, nstm_index, nstm_sq) = match stm {
                                Color::White => {
                                    (index, sq as usize, ((index + 6) % 12), sq as usize ^ 56)
                                }
                                Color::Black => {
                                    (((index + 6) % 12), sq as usize ^ 56, index, sq as usize)
                                }
                            };
                            self.boards_stm[self.counter] =
                                [batch_counter as i64, (stm_index * 64 + stm_sq) as i64];
                            self.boards_nstm[self.counter] =
                                [batch_counter as i64, (nstm_index * 64 + nstm_sq) as i64];
                            self.counter += 1;
                        }
                    }
